...
object ConfigBeanFactory {
  ...
  def create[T](config: Config, clazz: Class[T]): T =
    ConfigBeanImpl.createInternal(config, clazz, true)
   ...
   * {{{
   * val configSource: Config = ConfigFactory.load().getConfig("foo");
   * val config: FooConfig = ConfigBeanFactory.create(configSource, classOf[FooConfig], false);
   * }}}
   *
   * The Java class should follow JavaBean conventions. Field types can be any
   * of the types you can normally get from a [[Config]], including
   ...
   *   if false, throw a validation error when the config has keys that do not
   ...
   *   unknown config keys when `allowUnknownConfigKeys` is false
   * @throws ConfigException
   *   Can throw the same exceptions as the getters on [[Config]]
   */
  def create[T](
    ...
    ConfigBeanImpl.createInternal(config, clazz, allowUnknownConfigKeys)
}
